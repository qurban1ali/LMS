...
const CourseCard: FC<Props> = ({ item, isProfile }) => {
  ...
  return (
    <Link href={href} className={`block `}>
      <div className={`w-full min-h-[54vh] relative overflow-hidden rounded-lg border  shadow-sm hover:shadow-md transition-transform duration-150 ease-in-out hover:scale-[1.01] p-3 ${isDark ? 'bg-gray-900 text-white border-white' : 'bg-white text-black'}`}>
        {/* optional badge */}
        {discountLabel && (
        ...
        <div className={`mt-3`}>
          <h1 className={`font-Poppins text-[16px] font-semibold line-clamp-2`}>
          ...
          <div className={`flex items-center justify-between mt-3 `}>
            <div className={`flex items-baseline gap-2 `}>
              ...
              {item.estimatedPrice != null && item.price != null && item.estimatedPrice > item.price && (
                <div className={`text-[12px] line-through opacity-80 `}>
